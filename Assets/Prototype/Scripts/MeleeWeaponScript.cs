...
public class MeleeWeaponScript : MonoBehaviour {
    ...
    public void EquipWeapon() {
        ...
        if (isEquipped && equipCD < 0f && !isStuck)
        {
            ...
            playerCam.GetComponent<TestPickupScript>().CurrentWeapon = gameObject; // Makes this object be accessible through the "TestPickupScript" in Main Camera;
        }
        if (isStuck)
        {   // Maybe play a special animation where the player rips the weapon out of the object.
            ...
            playerCam.GetComponent<TestPickupScript>().CurrentWeapon = gameObject;
        }
        if (!isEquipped) {
    ...
    public void UnEquipWeapon() {
        if (equipCD < 0f)
        {
            ...
            playerCam.GetComponent<TestPickupScript>().CurrentWeapon = null;
        }
    }
